...
class DrawingView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    // Class để lưu thông tin hành động
    ...
    // Danh sách lưu lịch sử hành động
    ...
    private fun constrainPan() {
        ...
        // Giới hạn để canvas không di chuyển ra ngoài tầm nhìn
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        // Chuyển đổi tọa độ từ screen sang canvas
        ...
        val x = pts[0] // Tọa độ x trên canvas
        val y = pts[1] // Tọa độ y trên canvas
        ...
        if (isTextMode && currentText != null) {
            when (event.action and MotionEvent.ACTION_MASK) {
                ...
                MotionEvent.ACTION_UP -> {
                    ...
                    texts.add(textItem)
                    actions.add(Action(ActionType.TEXT, text = textItem)) // Ghi lại hành động
                    undoneTexts.clear()
        ...
        // Xử lý pixel mode
        if (isPixelMode) {
            // Chỉ xử lý vẽ pixel khi dùng một ngón tay và không zoom
            if (event.pointerCount == 1 && !scaleDetector.isInProgress) {
                when (event.action and MotionEvent.ACTION_MASK) {
                    MotionEvent.ACTION_DOWN, MotionEvent.ACTION_MOVE -> {
                        // Tính toán tọa độ pixel dựa trên lưới
            ...
            return true // Trả về true để xử lý zoom/multi-touch
        }
